...
public class IncomeStatementDaoImpl implements IncomeStatementDao {
	...
	public IncomeStatementEntity createIncomeStatement(IncomeStatementEntity incomestatement, String dataBaseName)
		...
		try {
			...
			boolean isMatched = existingValues.stream().anyMatch(incomestatement.getLineItem()::equalsIgnoreCase);
			System.out.println("value" + existingValues);
			if (isMatched == false && !existingValues.contains(incomestatement.getLineItem())) {
				...
				String id2 = incomestatement.getIncomeid();
				System.out.println(id1 + " " + incomestatement);
	...
	@Override
	public ArrayList<IncomeStatementEntity> uploadExcelSheetIncomeTable(CompletedFileUpload file, String dataBaseName)
			throws IncomeStatementDaoException {
		Connection connection = null;
		PreparedStatement pstmt = null;

...
public class InvestorDatabaseTables {
	...
	public static final String CREATE_USER_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.userTable(userid uniqueidentifier NOT NULL,firstName varchar(255) NOT NULL,lastName varchar(255) NOT NULL,email varchar(255) unique,mobileNumber varchar(255) unique,domain varchar(255) NOT NULL,assignedName varchar(255) NOT NULL,roleName varchar(255) NOT NULL foreign key references #$dataBaseName#$.dbo.rolemodel(roleName) on update cascade ,userStatus varchar(20),password varchar(200) ,editedOn bigint NOT NULL,createdOn bigint NOT NULL,CONSTRAINT userid PRIMARY KEY CLUSTERED(userid))";

	public static final String CREATE_BALANCESHEETFORM_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.balanceSheetform(balanceid uniqueidentifier NOT NULL,lineItem varchar(255),alternativeName varchar(255),CONSTRAINT balanceid PRIMARY KEY CLUSTERED(balanceid))";

	public static final String CREATE_INCOMESTATEMENT_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.incomestatement(incomeid uniqueidentifier NOT NULL,lineItem varchar(255),alternativeName varchar(255),CONSTRAINT incomeid PRIMARY KEY CLUSTERED(incomeid))";

	public static final String CREATE_CASHFLOW_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.cashflow(cashId  uniqueidentifier NOT NULL,lineItem varchar(255),alternativeName varchar(255),CONSTRAINT cashid PRIMARY KEY CLUSTERED(cashid))";

	public static final String CREATE_SHAREHOLDERDATAFORM_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.shareholderdataform(shareId uniqueidentifier NOT NULL,clientName varchar(255) NOT NULL,portfolioId varchar(255) NOT NULL,folio varchar(255) NOT NULL,shareholderName varchar(255) NOT NULL,holdingValue varchar(255) NOT NULL,holdingPercentage varchar(255) NOT NULL,holdingCost varchar(255) NOT NULL,minorCode varchar(255) NOT NULL,fundGroup varchar(255) NOT NULL,date varchar(255) NOT NULL,CONSTRAINT shareid PRIMARY KEY CLUSTERED(shareid))";
	...
	public static final String CREATE_NONPROCESS_FILETABLE = "CREATE TABLE #$dataBaseName#$.dbo.nonprocessFileTable(npFileId INT IDENTITY ,client varchar(255),clientId varchar(255),fileName varchar(255) NOT NULL,fileType varchar(255) NOT NULL,FileData VARBINARY(MAX),hashCode varchar(255),status varchar(255))";
	...
	public static final String CREATE_FORCAST_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.forecast(f_id INT IDENTITY,fileId INT FOREIGN KEY REFERENCES #$dataBaseName#$.dbo.dataIngestion(fileId),CMP_value varchar(255),target_price varchar(255),rating varchar(255),date varchar(255))";
	...
	public static final String CREATE_DATAINGESTION_MAPPING_TABLE = "CREATE TABLE #$dataBaseName#$.dbo.dataIngestionMappingtable(mapId uniqueidentifier NOT NULL,fieldId bigint NOT NULL,analyst varchar(255) ,masterLineItem varchar(255),companyName varchar(255) ,tableName varchar(255),documentType varchar(255) ,peerName varchar(255),currency varchar(255),units varchar(255),year varchar(255) NOT NULL,lineItemName varchar(255) NOT NULL,quarter varchar(255) NOT NULL,type varchar(255) NOT NULL,value varchar(255),reportType varchar(255) NOT NULL,reportDate date NOT NULL,date bigint NOT NULL,denomination varchar(255),CONSTRAINT mapId PRIMARY KEY CLUSTERED(mapId))";
	...
	public static final String CREATE_MEETINGSHEDULAR = "CREATE TABLE #$dataBaseName#$.dbo.meetingshedulartable(meetingSheduleId bigint IDENTITY ,meetingId varchar(max) NOT NULL,eventId varchar(max) NOT NULL,joinUrl varchar(max) NOT NULL,title varchar(255) NOT NULL,agenda varchar(255) NOT NULL,participant varchar(255) NOT NULL,meetingDate varchar(255) NOT NULL,startTime varchar(255) NOT NULL,endTime varchar(255) NOT NULL,meetingType varchar(255) NOT NULL,recordAutomatically bit,meetingDataStatus varchar(255))";

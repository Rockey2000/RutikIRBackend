...
public class AnalystLineItemController {
	...
	public MutableHttpResponse<?> uploadAnalystLineItemExcelSheet(CompletedFileUpload file)
			throws AnalystLineItemControllerException {
		try {

			ArrayList<AnalystLineItemEntity> response = this.analystlineitemservice.uploadAnalstLineItemExcelSheet(file);
			if(response.isEmpty())
			{

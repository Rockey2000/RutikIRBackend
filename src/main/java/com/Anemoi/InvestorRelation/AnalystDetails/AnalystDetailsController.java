...
public class AnalystDetailsController {
	...
	@Delete("/{analystId}")
	public HttpResponse<AnalystDetailsEntity> deleteAnalystDetails(@PathVariable("analystId")long analystId)
			throws AnalystDetailsControllerException {
		try {
	...
	public MutableHttpResponse<?> uploadExcelSheetAnalystDetilas(CompletedFileUpload file)
			throws AnalystDetailsControllerException {
		try {
			ArrayList<AnalystDetailsEntity> response = this.service.uploadAnalstDetailsExcelSheet(file);
			if(response.isEmpty())
			{

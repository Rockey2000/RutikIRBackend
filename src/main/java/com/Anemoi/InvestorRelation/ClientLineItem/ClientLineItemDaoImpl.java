...
public class ClientLineItemDaoImpl implements ClientLineItemDao {
	...
	@SuppressWarnings("resource")
	@Override
	public ArrayList<ClientLineItemEntity> uploadClientLineItem(CompletedFileUpload file, String dataBaseName) throws ClientLineItemDaoException {
		Connection connection = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String lineItemName;
...
//						System.out.println(Analystname+ "this analystname not present in analyst details table");
			...
			for (Row row1 : sheet) {
				...
				String lineName = (formatter.formatCellValue(row1.getCell(1)).trim());
				if (!lineName.isEmpty()) {
				...
				String clientTableheaderName = (formatter.formatCellValue(row1.getCell(3)).trim());
				if (clientTableheaderName.isEmpty()) {

					throw new ClientLineItemDaoException("clientTableheaderName  is empty ");

				}
				String masterTableSource = (formatter.formatCellValue(row1.getCell(4)).trim());

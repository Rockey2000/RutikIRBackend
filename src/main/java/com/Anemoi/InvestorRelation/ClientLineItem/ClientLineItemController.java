...
public class ClientLineItemController {
	...
	public HttpResponse<?> uploadClientLineItemExcelSheet(CompletedFileUpload file)
			throws ClientLineItemControllerException {
		try {

			ArrayList<ClientLineItemEntity> response = this.clientLineItemService.uploadClientLineItemExcelSheet(file);
			if(response.isEmpty())
			{
